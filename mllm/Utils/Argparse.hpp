  ...
  static void parse(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < args.size(); ++i) {
      ...
      if (arg[0] == '-') {
        ...
        for (auto& param : inst.args_) {
          ...
          if (std::find(flags.begin(), flags.end(), arg) != flags.end()) {
            if (param->isBoolean()) {
              ...
              if (i + 1 >= args.size()) { MLLM_ERROR_EXIT(kError, "Missing value for {}", arg); }
              param->parse(args[++i]);
            }
            found = true;
            break;
        ...
        if (!found) { MLLM_ERROR_EXIT(kError, "Unknown option: {}", arg); }
      } else {
    ...
    for (auto& param : inst.args_) {
      if (param->isPositional()) {
        if (pos_idx >= positional_args.size()) {
          if (param->isRequired()) { MLLM_ERROR_EXIT(kError, "Missing positional argument"); }
          continue;
        }
    ...
    for (auto& param : inst.args_) {
      if (param->isRequired() && !param->isSet()) {
        MLLM_ERROR_EXIT(kError, "Missing required argument: {}", param->meta());
      }
    }
  }
  ...
  static T get(const std::string& flag) {
    ...
    for (auto& param : inst.args_) {
      ...
      if (std::find(flags.begin(), flags.end(), flag) != flags.end()) {
        ...
        if (p) { return p->get(); }
        MLLM_ERROR_EXIT(kError, "Type mismatch for {}", flag);
      }
    }
    MLLM_ERROR_EXIT(kError, "Argument not found: {}", flag);
  }
