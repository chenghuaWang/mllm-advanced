...
#ifdef _MSC_VER
#error \
    "The Mllm's Dbg helper is built on GCC/Clang extension, which is not compatible with MSVC compiler."
#endif
...
#define MLLM_DBG_IMPL_TYPE_(...)                                                                 \
  __MLLM_DBG_IMPL_TYPE_PRIVATE(0, ##__VA_ARGS__, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, \
                               2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, \
                               2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, \
                               2, 2, 2, 2, 0)

#define __MLLM_DBG_IMPL_TYPE_PRIVATE(                                                              \
...
std::vector<std::string> _mllm_dbg_split_args_name(const char* args_name) {
  ...
  token.erase(token.begin(), std::find_if(token.begin(), token.end(),
                                          [](unsigned char ch) { return !std::isspace(ch); }));
  token.erase(
      std::find_if(token.rbegin(), token.rend(), [](unsigned char ch) { return !std::isspace(ch); })
          .base(),
      token.end());
