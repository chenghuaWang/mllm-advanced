...
#include "mllm/Utils/SymExpr/Tokens.hpp"
#include "fmt/core.h"
#include <cmath>
#include <cstdint>
#include <cstdlib>
...
class SymbolExprAST {
 public:
  ...
  int32_t evalAsInt(const std::unordered_map<std::string, float>& var_values) {
    return static_cast<int32_t>(eval(var_values));
  }
...
class VarAST final : public SymbolExprAST {
 public:
  ...
  float eval(const std::unordered_map<std::string, float>& var_values) override {
    return var_values.find(var_name_)->second;
  }
