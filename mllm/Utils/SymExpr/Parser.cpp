...
bool SymbolExprTokenDic::haveToken(const std::string& name) const {
...
SymbolExprParser::expr_ast_ptr_t SymbolExprParser::parseBinaryExpr(expr_ast_ptr_t& prev_expr) {
  ...
  if (next_op.type_ == SymbolExprTokenType::Operators
      && lexer_->getPrecedence(next_op) > lexer_->getPrecedence(binary_op)) {
    (void)getNextToken();
