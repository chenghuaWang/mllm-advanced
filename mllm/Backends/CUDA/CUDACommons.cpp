...
GpuMetaInfo::GpuMetaInfo() {
  MLLM_CHECK_NVML_ERROR(nvmlInit());
  ...
  MLLM_CHECK_NVML_ERROR(nvmlDeviceGetCount(&device_count));
  ...
  for (unsigned int i = 0; i < device_count; ++i) {
    ...
    int sm_count;
    cu_result =
        cuDeviceGetAttribute(&sm_count, CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT, cu_devices);
    if (cu_result == CUDA_SUCCESS) {
    ...
    CUresult cu_result_major = cuDeviceGetAttribute(
        &compute_cap_major, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR, cu_devices);
    CUresult cu_result_minor = cuDeviceGetAttribute(
        &compute_cap_minor, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR, cu_devices);
    ...
    int l1_cache;
    cu_result =
        cuDeviceGetAttribute(&l1_cache, CU_DEVICE_ATTRIBUTE_LOCAL_L1_CACHE_SUPPORTED, cu_devices);
    if (cu_result == CUDA_SUCCESS) {
    ...
    cu_result = cuDeviceGetAttribute(
        &shared_mem_per_sm, CU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_MULTIPROCESSOR, cu_devices);
    if (cu_result == CUDA_SUCCESS) {
    ...
    cu_result = cuDeviceGetAttribute(
        &max_thread_per_sm, CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_MULTIPROCESSOR, cu_devices);
    if (cu_result == CUDA_SUCCESS) {
    ...
    cu_result =
        cuDeviceGetAttribute(&warp_size_in_thread, CU_DEVICE_ATTRIBUTE_WARP_SIZE, cu_devices);
    if (cu_result == CUDA_SUCCESS) {
    ...
    int architecture;
    cu_result = cuDeviceGetAttribute(&architecture, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR,
                                     cu_devices);
    if (cu_result == CUDA_SUCCESS) {
