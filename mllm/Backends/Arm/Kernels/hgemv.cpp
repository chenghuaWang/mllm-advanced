...
void hgemv_1K_NK_V1(const float16_t* __restrict A, const float16_t* __restrict B,
  ...
  // the tiling method is different from single threads for thread safe.
  if (threads) {
...
void hgemv_1K_NK_V2_HP(const float16_t* __restrict A, const float16_t* __restrict B,
  ...
  if (bias) {
    ...
    std::fill(C_fp32.begin(), C_fp32.end(), 0.f);
  }
...
#pragma omp for schedule(auto)
      for (int n_block_idx = 0; n_block_idx < n_steps; ++n_block_idx) {
        ...
        for (int k_block_idx = 0; k_block_idx < k_steps; ++k_block_idx) {
          ...
          if (k_block_idx < k_steps - 1) {
            const float16_t* next_B_ptr = B + ns * K + (ks + K_TILE_SIZE);
...
#pragma unroll
          ...
          float32x4_t sum0;
          sum0 = vsetq_lane_f32(vaddvq_f32(acc0), sum0, 0);
          sum0 = vsetq_lane_f32(vaddvq_f32(acc1), sum0, 1);
          sum0 = vsetq_lane_f32(vaddvq_f32(acc2), sum0, 2);
          sum0 = vsetq_lane_f32(vaddvq_f32(acc3), sum0, 3);
