...
/** Note that the order of declarations given here defines the order in which ops and graph
 * optimizations are registered to the HTP Core. Append the latest OpName at the bottom
 */
...
static std::array<const char*, 1> sg_opNames{{"SiLU"}};
 ...
 * can be set by Qnn_ErrorHandle_t MllmPackageLogSetLevel(QnnLog_Level_t maxLogLevel)
 */
/*
...
Qnn_ErrorHandle_t MllmPackageInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t MllmPackageGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t MllmPackageLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t MllmPackageLogSetLevel(QnnLog_Level_t maxLogLevel) {
  if (maxLogLevel < QNN_LOG_LEVEL_ERROR) return QNN_LOG_ERROR_INVALID_ARGUMENT;
  sg_maxLogLevel = maxLogLevel;
  return QNN_SUCCESS;
}

Qnn_ErrorHandle_t MllmPackageLogTerminate() {
...
Qnn_ErrorHandle_t MllmPackageValidateOpConfig(Qnn_OpConfig_t opConfig) {
 ...
 * no auto-generated implementations are created. Users should see example for full function
 *signatures. (version 1.3.0) Qnn_ErrorHandle_t MllmPackageCreateKernels
 *(QnnOpPackage_GraphInfrastructure_t graphInfrastructure, QnnOpPackage_Node_t node,
 *QnnOpPackage_Kernel_t** kernels, uint32_t* numKernels) (version 1.3.0) Qnn_ErrorHandle_t
 *MllmPackageFreeKernels (QnnOpPackage_Kernel_t* kernels)
 *
 * (version 1.4.0) Qnn_ErrorHandle_t MllmPackageCreateOpImpl (QnnOpPackage_GraphInfrastructure_t
 * graphInfrastructure, QnnOpPackage_Node_t node, QnnOpPackage_OpImpl_t* opImpl)
 *(version 1.4.0) Qnn_ErrorHandle_t MllmPackageFreeOpImpl (QnnOpPackage_OpImpl_t opImpl)
 */

Qnn_ErrorHandle_t MllmPackageTerminate() {
  if (!sg_packageInitialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
/* latest version */
Qnn_ErrorHandle_t MllmPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
  if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
  interface->interfaceVersion = {1, 4, 0};
  interface->v1_4.init = MllmPackageInit;
  interface->v1_4.terminate = MllmPackageTerminate;
  interface->v1_4.getInfo = MllmPackageGetInfo;
  interface->v1_4.validateOpConfig = MllmPackageValidateOpConfig;
  interface->v1_4.createOpImpl = nullptr;
  interface->v1_4.freeOpImpl = nullptr;
  interface->v1_4.logInitialize = MllmPackageLogInitialize;
  interface->v1_4.logSetLevel = MllmPackageLogSetLevel;
  interface->v1_4.logTerminate = MllmPackageLogTerminate;
  return QNN_SUCCESS;
}
