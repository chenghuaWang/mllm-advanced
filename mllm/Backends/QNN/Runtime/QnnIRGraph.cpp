...
void QnnIRGraph::setupInputs(const std::vector<ir::tensor::TensorValue::self_ptr_t>& inputs) {
  for (auto& input_tensor_ir : inputs) {
    auto mllm_tensor = input_tensor_ir->tensor_;

    Qnn_DataType_t qnn_dtype;
    auto quant_defined = QNN_DEFINITION_UNDEFINED;
    auto quant_type = QNN_QUANTIZATION_ENCODING_UNDEFINED;

    // HTP Only support this.
    float scale = 0.f;

    switch (mllm_tensor.dtype()) {
      case kFp32: qnn_dtype = QNN_DATATYPE_FLOAT_32; break;
      case kFp16: qnn_dtype = QNN_DATATYPE_FLOAT_16; break;
      case kPTInt8_Sym: {
        qnn_dtype = QNN_DATATYPE_SFIXED_POINT_8;
        quant_defined = QNN_DEFINITION_DEFINED;
        quant_type = QNN_QUANTIZATION_ENCODING_SCALE_OFFSET;

        // scale tensor's shape is [1]
        scale = mllm_tensor.getExtraTensorViewInTensor("scale").at<float>({0});
        break;
      }
    ...
    auto qnn_tensor = Qnn_Tensor_t{.version = QNN_TENSOR_VERSION_2,
                                   .v2 =
                                       {// Unique integer identifier for a tensor, generated by
                                        // the backend based on the tensor name.
                                        .id = 0,
                                        .name = input_tensor_ir->name().c_str(),
                                        .type = QNN_TENSOR_TYPE_APP_WRITE,
                                        .dataFormat = QNN_TENSOR_DATA_FORMAT_FLAT_BUFFER,
                                        .dataType = qnn_dtype,
                                        .quantizeParams = {quant_defined,
                                                           quant_type,
                                                           {.scaleOffsetEncoding =
                                                                {
                                                                    .scale = scale,
...
void QnnIRGraph::startRecord() {
  ...
  auto status = qnn_func_symbols_.qnn_interface_.graphCreate(
      qnn_bk_device_.qnn_ctx_handle_, name_.c_str(), (const QnnGraph_Config_t**)&graph_cfg,
      &qnn_graph_handle_);
