/**
 * @file image_preprocessor_qwen2vl.hpp
...
class Qwen2VLImagePreprocessor {
 public:
  inline explicit Qwen2VLImagePreprocessor(int min_pixels = 56 * 56,
                                           int max_pixels = 14 * 14 * 4 * 1280)
      : min_pixels_(min_pixels), max_pixels_(max_pixels) {
    OPENAI_CLIP_MEAN = Tensor::empty({3, 1, 1}, kFp32, kCPU).alloc();
    OPENAI_CLIP_STD = Tensor::empty({3, 1, 1}, kFp32, kCPU).alloc();
  ...
  inline std::pair<int32_t, int32_t> smartResize(int height, int width, int factor = 28,
                                                 int min_pixels = 56 * 56,
                                                 int max_pixels = 14 * 14 * 4 * 1280) {
    if (std::max(height, width) / static_cast<double>(std::min(height, width)) > 200.0) {
      MLLM_ERROR_EXIT(kError, "absolute aspect ratio must be smaller than 200");
    }
  ...
  inline std::pair<Tensor, std::array<int32_t, 3>> operator()(const std::string& image_path) {
      ...
      for (int h = 0; h < new_h; ++h) {
        for (int w = 0; w < new_w; ++w) {
          *patches.offsettedPtr<float>({c, h, w}) =
              (*patches.offsettedPtr<float>({c, h, w}) - OPENAI_CLIP_MEAN.ptr<float>()[c])
              / (OPENAI_CLIP_STD.ptr<float>()[c]);
        }
      }
    }
    ...
    patches =
        patches.view({grid_t, temporal_patch_size_, channel, grid_h / merge_size_, merge_size_,
