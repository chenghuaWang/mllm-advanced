...
static void hgemv_v1_4_threads(benchmark::State& state) {
  mllm::Logger::level() = mllm::LogLevel::kError;
  ...
  for (auto _ : state) {
    hgemv_1K_NK_V1((float16_t*)A, (float16_t*)B, nullptr, (float16_t*)C, size, size, 4);
  }
...
static void hgemv_v2_hp_4_threads(benchmark::State& state) {
  mllm::Logger::level() = mllm::LogLevel::kError;
  ...
  for (auto _ : state) {
    hgemv_1K_NK_V2_HP((float16_t*)A, (float16_t*)B, nullptr, (float16_t*)C, size, size, 4);
  }
...
static void sgemv_v1_4_threads(benchmark::State& state) {
  size_t size = state.range(0);
  ...
  for (auto _ : state) { sgemv_1K_NK_V1((float*)A, (float*)B, nullptr, (float*)C, size, size, 4); }
...
BENCHMARK(hgemv_v1_4_threads)->RangeMultiplier(2)->Range(64, 2048);
BENCHMARK(hgemv_v2_hp)->RangeMultiplier(2)->Range(64, 2048);
BENCHMARK(hgemv_v2_hp_4_threads)->RangeMultiplier(2)->Range(64, 2048);
BENCHMARK(sgemv_v1)->RangeMultiplier(2)->Range(64, 2048);
BENCHMARK(sgemv_v1_4_threads)->RangeMultiplier(2)->Range(64, 2048);
BENCHMARK_MAIN();
