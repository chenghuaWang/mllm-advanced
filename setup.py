...
class CustomBuild(build):
    ...
    def run(self):
        ...
        if "CMAKE_BUILD_ARGS" in os.environ:
            build_args += [
                item
                for item in os.environ["CMAKE_BUILD_ARGS"].split(" ")
                if item  # noqa: E501
            ]
        ...
        cmake_cache_dir = os.path.join(repo_root, self.build_base, "cmake-out")
...
def get_ext_modules() -> List[Extension]:
    ...
    # _C lib
    ext_modules.append(BuiltExtention("_C.*", "mllm._C"))
    return ext_modules
